...
?>
	...
	$listar = null;
	$directorio = opendir("../../public/documents/documentos/VRevicion/");
	$borrarFor=null;

	while ($elemento = readdir($directorio)) {
		if ($elemento != '.' && $elemento != '..') {
			if (is_dir("../../public/documents/documentos/" . $elemento)) {
				$listar .= "<a class=' col-md-6 btn btn-primary block' href='../../public/documents/documentos/VRevicion/$elemento' target='_blank'> 
      $elemento/</a>
      <br><br>";
			} else {
				$listar .= "<a class=' col-md-6 btn btn-primary block' href='../../public/documents/documentos/VRevicion/$elemento' target='_blank'> 
	...
	if ($_POST["subir"] == "Cargar archivo") {
		$folder = "../../public/documents/documentos/VRevicion/";
	...
	if (isset($_POST['borrar'])) {
		@unlink('../../public/documents/documentos/VRevicion/' . $borrarFor);
		echo "
	  <div class='alert alert-danger'><p class='hidd' align=center>El archivo  " . $_FILES["formato"]["name"] . " ha sido eliminado correctamente. <a href='index.php' class='btn btn-default'>Cliqué aquí </a> para verificar.</div>";
		...
		/////////////ENLISTAR LOS FICHEROS EXISTENTES///////////////////////////////////////////////
		$listarv = null;
		$directoriov = opendir("../../public/videos/VRevicion/");
	
		while ($elemento = readdir($directoriov)) {
			if ($elemento != '.' && $elemento != '..') {
				if (is_dir("../../public/documents/documentos/" . $elemento)) {
					$listarv .= "<a class=' col-md-6 btn btn-primary block' href='../../public/videos/VRevicion/$elemento' target='_blank'> 
		  $elemento/</a>
		  <br><br>";
				} else {
					$listarv .= "<a class=' col-md-6 btn btn-primary block' href='../../public/videos/VRevicion/$elemento' target='_blank'> 
		...
		if ($_POST["subir"] == "Cargar video") {
			$folder = "../../public/videos/VRevicion/";
			move_uploaded_file($_FILES["formato"]["tmp_name"], "$folder" . $_FILES["formato"]["name"]);
			echo "
		  <div class='alert alert-success'><p class='hidd' align=center>El video  " . $_FILES["formato"]["name"] . " se ha cargado correctamente.<a href='index.php' class='btn btn-default'>Cliqué aquí </a> para verificar.</div>";
		}
		...
		if (isset($_POST['borrarv'])) {
			@unlink('../../public/videos/VRevicion/' . $borrarForv);
			echo "
		  <div class='alert alert-danger'><p class='hidd' align=center>El archivo  " . $_FILES["formato"]["name"] . " ha sido eliminado correctamente. <a href='index.php' class='btn btn-default'>Cliqué aquí </a> para verificar.</div>";
		}
	...
	<title>CRL-SENA</>::Revisión por la dirección</title>
	</head>
							...
							<section id="content-types">
									...
									<div class="col-md-6 col-sm-12">
										<div class="card">
											<div class="card-content">
												<div class="card-body">
													<h4 class="card-title">Formato de descarga Revisión por la dirección</h4>
								...
								<div class="col-md-6 col-sm-12">
									<div class="card">
										<div class="card-content">
											<div class="card-body">
											<h4 class="card-title">Formato guia de descarga Revisión por la dirección</h4>
												<form method="post" enctype="multipart/form-data" ">
													<div class="" style=" margin-top:2%; padding:3%; border-radius:20px;">
													<input class="form-control" type="file" name="formato" id="formato" style="margin-bottom:2%;">
